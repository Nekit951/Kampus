...
class TinyDB(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<ItemsModel?> {
        ...
        for (jObjString in objStrings) {
            val player: ItemsModel? = gson.fromJson<ItemsModel?>(jObjString, ItemsModel::class.java)
            playerList.add(player)
        }
        return playerList
    }

    fun getListOrder(key: String?): ArrayList<OrderModel?> {
        val gson = Gson()
        ...
        for (jObjString in objStrings) {
            val order: OrderModel? = gson.fromJson<OrderModel?>(jObjString, OrderModel::class.java)
            orderList.add(order)
        }
        return orderList
    }
    ...
    fun putListOrder(key: String, list: ArrayList<OrderModel?>) {
        checkForNullKey(key)
        val gson = Gson()
        val objStrings = ArrayList<String?>()
        for (order in list) {
